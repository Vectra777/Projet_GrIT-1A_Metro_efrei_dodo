...
const LIGHT_TILE_URL = "https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}.png";
const DARK_TILE_URL = "https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png";
const CARBON_FACTORS_KG_PER_PASSENGER_KM = {
  metro: 0.0042,
  rer: 0.0042,
};
...
function applyTheme(theme) {
  ...
  els.themeToggle.title = isDark ? "Passer en mode clair" : "Passer en mode sombre";
}
...
function drawMapBackground(ctx) {
  ...
  ctx.globalAlpha = darkMode ? 0.78 : 0.62;
  ...
  ctx.globalAlpha = 1;
  ctx.fillStyle = darkMode ? "rgba(5, 9, 16, 0.18)" : "rgba(255, 255, 255, 0.34)";
  ctx.fillRect(0, 0, rect.width, rect.height);
}
...
async function compareAlgorithms() {
  ...
  renderComparison(dijkstraResult, dijkstraMs, astarResult, astarMs, startAbs);
  draw();
}
...
function dijkstra(fromStation, toStation, startAbs) {
  ...
  legs.reverse();
  return { algorithm: "Dijkstra", arrival: dist[bestStop], legs, exploredStops };
}

function astar(fromStation, toStation, startAbs) {
  const stops = state.network.stops;
  const stations = state.network.stations;
  const targetStops = stations[toStation].stops;
  const heuristic = buildStopHeuristic(targetStops);
  const dist = new Float64Array(stops.length);
  dist.fill(Number.POSITIVE_INFINITY);
  const previous = new Array(stops.length);
  const queue = new PriorityQueue();
  ...
  for (const stopIndex of stations[fromStation].stops) {
    dist[stopIndex] = startAbs;
    queue.push(stopIndex, startAbs + heuristic[stopIndex]);
  }
  ...
  while (queue.size > 0) {
    ...
    if (!current) continue;
    const stopIndex = current.node;
    if (dist[stopIndex] + heuristic[stopIndex] !== current.priority) continue;
    exploredStops++;
    if (stops[stopIndex].station === toStation) {
      bestStop = stopIndex;
      break;
    ...
    for (const transfer of state.outgoingTransfers[stopIndex]) {
      const toStop = transfer[1];
      const arrival = dist[stopIndex] + transfer[2];
      if (arrival < dist[toStop]) {
        dist[toStop] = arrival;
        previous[toStop] = { type: "transfer", from: stopIndex, seconds: transfer[2] };
        queue.push(toStop, arrival + heuristic[toStop]);
      }
    }

    for (const edge of state.outgoingTransit[stopIndex]) {
      const schedule = nextSchedule(edge, dist[stopIndex]);
      if (!schedule) continue;
      const toStop = edge[1];
      if (schedule.arrival < dist[toStop]) {
        dist[toStop] = schedule.arrival;
        ...
        queue.push(toStop, schedule.arrival + heuristic[toStop]);
      }
    }
  }
  ...
  legs.reverse();
  return { algorithm: "A*", arrival: dist[bestStop], legs, exploredStops };
}

function buildStopHeuristic(targetStops) {
  const maxSpeedMetersPerSecond = 83.33;
  return state.network.stops.map((stop) => {
    const bestDistance = Math.min(
      ...targetStops.map((targetStop) => stopDistanceMeters(stop, state.network.stops[targetStop])),
    );
    return bestDistance / maxSpeedMetersPerSecond;
  });
}

function stopDistanceMeters(from, to) {
...
function renderRouteResult(result, startAbs) {
  ...
  const carbon = calculateCarbonFootprint(result);
  ...
  els.routeResult.innerHTML = `
    ...
    <div class="route-meta">${escapeHtml(from)} → ${escapeHtml(to)} · départ ${formatClock(startAbs)} · ${escapeHtml(result.algorithm)}</div>
    <div class="carbon-result">
      <strong>${formatCarbon(carbon.totalKg)} CO₂e</strong>
      <span class="route-meta">${formatDistance(carbon.totalKm)} parcourus · ${escapeHtml(CARBON_SOURCE)}</span>
    </div>
    ${stepsHtml}
  `;
}

function updateRouteHighlight(result) {
  const routeEdges = [];
...
function renderComparison(dijkstraResult, dijkstraMs, astarResult, astarMs, startAbs) {
  const reference = dijkstraResult || astarResult;
  updateRouteHighlight(reference);
  const from = state.network.stations[Number(els.fromStation.value)].name;
  const to = state.network.stations[Number(els.toStation.value)].name;
  const sameArrival = dijkstraResult && astarResult && dijkstraResult.arrival === astarResult.arrival;
  const note = sameArrival
    ? "Les deux algorithmes trouvent la même arrivée."
    : "Les résultats diffèrent, Dijkstra reste la référence.";

  els.routeResult.innerHTML = `
    <div class="route-title">
      <span>Comparaison</span>
      <span>${sameArrival ? "OK" : "Écart"}</span>
    </div>
    <div class="route-meta">${escapeHtml(from)} → ${escapeHtml(to)} · départ ${formatClock(startAbs)}</div>
    <div class="comparison">
      ${renderAlgorithmComparison("Dijkstra", dijkstraResult, dijkstraMs, startAbs)}
      ${renderAlgorithmComparison("A*", astarResult, astarMs, startAbs)}
    </div>
    <div class="route-meta">${note}</div>
  `;
}

function renderAlgorithmComparison(name, result, elapsedMs, startAbs) {
  if (!result) {
    return `
      <div class="comparison-row">
        <strong>${escapeHtml(name)}</strong>
        <span class="route-meta">Aucun trajet trouvé.</span>
      </div>
    `;
  }
  const carbon = calculateCarbonFootprint(result);
  return `
    <div class="comparison-row">
      <strong>${escapeHtml(name)} · ${formatDuration(result.arrival - startAbs)} · arrivée ${formatClock(result.arrival)}</strong>
      <span class="route-meta">${result.exploredStops} arrêts explorés · ${elapsedMs.toFixed(1)} ms · ${formatCarbon(carbon.totalKg)} CO₂e · ${formatDistance(carbon.totalKm)}</span>
    </div>
  `;
}

function calculateCarbonFootprint(result) {
  const byMode = {
    metro: { km: 0, kg: 0 },
    rer: { km: 0, kg: 0 },
  };
  ...
  for (const leg of result.legs) {
    ...
    const mode = route.mode === "rer" ? "rer" : "metro";
    const fromStop = state.network.stops[leg.from];
    const toStop = state.network.stops[leg.to];
    const km = stopDistanceMeters(fromStop, toStop) / 1000;
    const factor = CARBON_FACTORS_KG_PER_PASSENGER_KM[mode] ?? CARBON_FACTORS_KG_PER_PASSENGER_KM.metro;
    const kg = km * factor;
    totalKm += km;
    totalKg += kg;
    byMode[mode].km += km;
